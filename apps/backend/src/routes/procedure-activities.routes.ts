...
import type { ProcedureActivitiesService } from "../services/procedure-activities.service.js";
...
export function procedureActivitiesRoutes(service: ProcedureActivitiesService): Router {
  const router = Router();
  ...
  router.get("/", async (req, res) => {
    try {
      ...
      console.error("[procedure-activities:list]", e);
  ...
  router.get("/:id", async (req, res) => {
    try {
      ...
      console.error("[procedure-activities:getById]", e);
      const err = errorResponse(e);
      res.status(err.status).json(err.body);
    }
  });
  ...
  router.post("/", async (req, res) => {
    try {
      if (!req.body.nameProcedureActivities?.trim())
      ...
      console.error("[procedure-activities:create]", e);
      const err = errorResponse(e);
      res.status(err.status).json(err.body);
    }
  });
  ...
  router.put("/:id", async (req, res) => {
    try {
      ...
      console.error("[procedure-activities:update]", e);
      const err = errorResponse(e);
      res.status(err.status).json(err.body);
    }
  });
  ...
  router.delete("/:id", async (req, res) => {
    try {
      ...
      console.error("[procedure-activities:delete]", e);
      const err = errorResponse(e);
      res.status(err.status).json(err.body);
    }
  });
