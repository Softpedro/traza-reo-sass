...
import {
  WaterFootprintError,
  type WaterFootprintService,
} from "../services/water-footprint.service.js";
import { errorResponse } from "../lib/http-error.js";

/** Número obligatorio y > 0 (peso y consumo de agua son DECIMAL NOT NULL). */
...
export function waterFootprintRoutes(service: WaterFootprintService): Router {
  ...
  // GET /api/water-footprints → huellas cargadas, con su orden de pedido y marca.
  router.get("/", async (_req, res) => {
    try {
      res.json(await service.list());
    } catch (e) {
      sendError(res, e, "[water-footprints:list]");
    }
  });
  ...
  router.get("/:id", async (req, res) => {
    try {
      ...
      if (!row) {
        return res.status(404).json({ error: "Huella hídrica no encontrada", type: "NOT_FOUND" });
      }
      res.json(row);
    } catch (e) {
      sendError(res, e, "[water-footprints:getById]");
    }
  });
  ...
  router.get("/:id/report", async (req, res) => {
    try {
      ...
      sendError(res, e, "[water-footprints:report]");
    }
  });
  ...
  router.post("/", async (req, res) => {
    try {
      ...
      const waterFootprint = parsePositiveNumber(body.waterFootprint);
      if (waterFootprint == null) {
        return res
          .status(400)
          .json({ error: "El consumo de agua debe ser un número mayor a 0", type: "VALIDATION" });
      }
      ...
      const created = await service.create({
        ...
        stateWaterFootprint:
          body.stateWaterFootprint === undefined
            ? undefined
            : Number(body.stateWaterFootprint),
        codUsuarioCargaDl:
          body.codUsuarioCargaDl != null ? String(body.codUsuarioCargaDl) : undefined,
      });
      res.status(201).json(created);
    } catch (e) {
      sendError(res, e, "[water-footprints:create]");
    }
  });
  ...
  router.put("/:id", async (req, res) => {
    try {
      ...
      const patch: Parameters<WaterFootprintService["update"]>[1] = {};
      ...
      if (body.waterFootprint !== undefined) {
        const waterFootprint = parsePositiveNumber(body.waterFootprint);
        if (waterFootprint == null) {
          return res
            .status(400)
        ...
        patch.waterFootprint = waterFootprint;
      }
      ...
      if (body.stateWaterFootprint !== undefined) {
        patch.stateWaterFootprint = Number(body.stateWaterFootprint);
      }
      ...
      sendError(res, e, "[water-footprints:update]");
    }
  });
  ...
  router.delete("/:id", async (req, res) => {
    try {
      ...
      sendError(res, e, "[water-footprints:delete]");
    }
  });
