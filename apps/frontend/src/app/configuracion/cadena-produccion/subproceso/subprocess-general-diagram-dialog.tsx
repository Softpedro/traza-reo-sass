...
import { SubprocessGeneralChainFlow } from "./subprocess-general-chain-flow";
...
export function SubprocessGeneralDiagramDialog({
  open,
  onOpenChange}: SubprocessGeneralDiagramDialogProps) {
  ...
  const flowKey = useMemo(
    () =>
      `${sortedEslabones.map((e) => e.idDlkProductionChain).join("-")}|${processes.length}|${subprocesses.length}`,
    [sortedEslabones, processes.length, subprocesses.length]
  );
