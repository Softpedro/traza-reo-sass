...
import type { RutaRow } from "./columns";
import { RUTA_STATUS_OPTIONS, ACTIVO_OPTIONS } from "./constants";
...
export function RutaEstadoModal({ open, onOpenChange, order, onSuccess }: Props) {
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    try {
      const res = await apiFetch(
        `/api/order-heads/${order.idDlkOrderHead}/ruta-estado`,
        {
          method: "PUT",
      ...
      if (!res.ok) {
        ...
        throw new Error(err.error || "Error al actualizar el estado de la ruta");
      }
      onSuccess();
      onOpenChange(false);
    } catch (err) {
        ...
        <form onSubmit={onSubmit} className="space-y-4 pt-2">
          ...
          <div className="space-y-1.5">
            ...
            <p className="text-xs text-muted-foreground">
              ...
              Trazabilidad y deja de listarse aquí.
            </p>
          </div>
