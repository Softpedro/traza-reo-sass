...
import type { CarbonFootprint } from "./columns";
...
export function HuellaCo2Modal({ open, onOpenChange, mode, footprint, onSuccess }: Props) {
  ...
  useEffect(() => {
    ...
    apiFetch(`/api/carbon-footprints/${footprint.idDlkCarbonFootprint}`)
      .then((r) => r.json())
      .then((row: CarbonFootprint) => {
        setWeight(row.weight == null ? "" : String(row.weight));
        setCo2(row.carbonFootprint == null ? "" : String(row.carbonFootprint));
        setEstimated(row.estimatedValue === 1);
        setUnitDescription(row.unitDescription ?? "");
        setScope(row.scope ?? "");
        setState(row.stateCarbonFootprint ?? 1);
  ...
  const downloadReport = useCallback(async () => {
    if (!footprint) return;
    try {
      const res = await apiFetch(`/api/carbon-footprints/${footprint.idDlkCarbonFootprint}/report`);
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const res = isCreate
        ? await apiFetch("/api/carbon-footprints", {
            method: "POST",
            ...
            body: JSON.stringify({
              ...
              size,
              stateCarbonFootprint: 1,
            }),
          })
        : await apiFetch(`/api/carbon-footprints/${footprint?.idDlkCarbonFootprint}`, {
            method: "PUT",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ ...body, stateCarbonFootprint: state }),
          });

      if (!res.ok) {
        const err = await res.json().catch(() => ({}));
        throw new Error(err.error || "Error al guardar la huella de CO2");
      }
      onSuccess();
      onOpenChange(false);
    } catch (err) {
        ...
        <form onSubmit={onSubmit} className="space-y-3 pt-2">
          ...
          <div className="space-y-1.5">
            ...
            <textarea
              ...
              rows={3}
              maxLength={1000}
              placeholder="De la cuna a la puerta (Cradle-to-Gate). Cultivo, hilatura, tejeduría y confección"
              className="flex w-full rounded-md border border-input bg-background px-3 py-2 text-sm ring-offset-background placeholder:text-muted-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50"
            />
          </div>
